...
# [...] Config sections, ... are saved in section_names(type set).
section_names = set()
...
def main():
  ...
  train_parser.set_defaults(action=train) # bound function train with subparser 'train'
  ...
  for section_name in section_names:
    train_parser.add_argument('--'+section_name, nargs='+') # '+': All augments in the section are matched with section_name
...
def train(**kwargs):
  ...
  if not load:
    with open(os.path.join(save_dir, 'config.cfg'), 'w') as f:
      config.write(f)
  input_networks, networks = resolve_network_dependencies(config, network_class, network_list, {})#return set(), networks
