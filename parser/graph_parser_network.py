...
class GraphParserNetwork(BaseNetwork):
  ...
  def build_graph(self, input_network_outputs={}, reuse=True):
    ...
    token_weights3D = tf.expand_dims(token_weights, axis=-1) * tf.expand_dims(root_weights, axis=-2)# batch_size*bucket_size*bucket_size
    ...
    for i in six.moves.range(self.n_layers):
      ...
      with tf.variable_scope('RNN-{}'.format(i)):
        ...
        # layer = classifiers.hidden(layer, 10, hidden_func=tf.math.tanh)
    ...
    with tf.variable_scope('Classifiers'):
      if 'semrel' in output_fields:
        ...
        #outputs['semgraph'] = unlabeled_outputs
        self._evals.add('semgraph')
      elif 'semhead' in output_fields:
        vocab = output_fields['semhead']

        outputs[vocab.classname] = vocab.get_bilinear_discriminator(
          layer,
          token_weights=token_weights3D,
          reuse=reuse)
        ...
        #     token_weights=token_weights3D,
        #     reuse=reuse)
        # outputs['semhead'] = unlabeled_outputs
        # self._evals.add('semhead')
