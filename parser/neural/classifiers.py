...
def hidden(layer, hidden_size, hidden_func=nonlin.relu, hidden_keep_prob=1.):
  ...
  weights = tf.get_variable('Weights', shape=[input_size, hidden_size])#, initializer=tf.random_normal_initializer())
...
def hidden_w(layer, hidden_size, hidden_func=nonlin.relu, hidden_keep_prob=1.):
  """"""
...
def hiddens(layer, hidden_sizes, hidden_func=nonlin.relu, hidden_keep_prob=1.):
  ...
  for i, hidden_size in enumerate(hidden_sizes):
    weights.append(tf.get_variable('Weights-%d' % i, shape=[input_size, hidden_size]))#,initializer=tf.random_normal_initializer()))
