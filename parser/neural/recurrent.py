...
#===============================================================
def RNN(layer, recur_size, seq_lengths, conv_width=0, recur_func=nonlin.relu, conv_keep_prob=1., recur_keep_prob=1., recur_include_prob=1., **kwargs):
  """"""

  batch_size, bucket_size, input_size = nn.get_sizes(layer)
  conv_size = 1+(2*conv_width)

  weights = tf.get_variable('Weights', shape=[conv_size, input_size, recur_size],initializer=tf.glorot_normal_initializer())#,trainable=False)
  ...
  with tf.variable_scope('Loop'):
    ...
    weights = tf.get_variable('Weights', shape=[recur_size, recur_size], initializer=tf.zeros_initializer, trainable=False)
    ...
    def body(i, last_hidden, last_hidden_sequence):
      ...
      current_hidden = tf.where(i < tf.to_int32(seq_lengths), current_hidden, null_hidden)
      current_hidden_sequence = last_hidden_sequence.write(i, tf.where(i < tf.to_int32(seq_lengths), current_hidden, null_hidden))
...
def BiRNN(layer, recur_size, seq_lengths, recur_cell=LSTM, bilin=False, **kwargs):
  ...
  if recur_cell == RNN:
    final_states = tf.squeeze(tf.matmul(fw_hidden, locations, transpose_a=True), -1)
  else:
