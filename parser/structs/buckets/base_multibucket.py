...
class BaseMultibucket(object):
  ...
  def get_len2bkt(max_lengths):
    """
    input: max_lengths:(list) list of maximum length of sentences allowed in each bucket

    output: len2bkt:(dict) a dictionary with (key,value) pair:
          (sentence_length, belong_to_which_bucket)
    """
  ...
  def compute_max_lengths(lengths, max_buckets):
    """
    inputs: 
    lengths -> (list) list of length of each sentence
    max_buckets -> (int) number of buckets

    output:
    splits: boundary length for each bucket.
    For example: input -> ([1,2,3,4],3)
                 output -> [1,2,4]
    indicates that sentences with length (0,1], (1,2], (2,4] forms three buckets.
    """
    
    uniq_lengths, count_lengths = np.unique(lengths, return_counts=True)
    # to compute pdf, every sentence is weighted by its length 
    ...
    for boundary, scores in enumerate(dists): 
      # every bucket. boudary: max length of sentence of current bucket
      for length, dist in enumerate(scores): 
        # every unique length. append the score w.r.t. each bucket
        all_scores.append((dist, boundary, length))
    # Sort all scores according to distance
    # (smaller distance, more likely the length belongs to the bucket)
    all_scores.sort()
