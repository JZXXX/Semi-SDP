...
def main():
  ...
  if not os.path.exists(write_file):
    with open(write_file,'w') as fw:
      fw.write('Unlabel score'+'\t'+'Label score'+'\n')
      fw.write('{:0.2f}'.format(UAS.F1*100)+'\n'+'{:0.2f}'.format(LAS.F1*100)+'\n')
  else:
    with open(write_file, 'a') as fw:
      fw.write('{:0.2f}'.format(UAS.F1*100)+'\n'+'{:0.2f}'.format(LAS.F1*100)+'\n')
  if 'ood' in gold_files:
    fw.write('\n')
...
#   print('Pr:{:0.2f},Re:{:0.2f},Acc:{0.2f}'.format(UAS.precision*100,UAS.recall*100,UAS.F1*100))
  #print(UAS.F1, UAS.seq_acc)
  #print('{:0.1f}'.format(LAS.F1*100))
#   print('{:0.1f}'.format(UAS.F1*100))
